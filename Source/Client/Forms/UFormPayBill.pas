...
type
  TfFormPayBill = class(TfFormNormal)
...
//------------------------------------------------------------------------------
class function TfFormPayBill.CreateForm(const nPopedom: string;
...
begin 
  ...
  with TfFormPayBill.Create(Application) do
  try
    Caption := '发票记录';
    EditKPDate.Date := Now;
    if Assigned(nP) then
...
class function TfFormPayBill.FormID: integer;
begin
  Result := cFI_FormPayBill;
end;

procedure TfFormPayBill.FormCreate(Sender: TObject);
begin
  LoadFormConfig(Self);
end;

procedure TfFormPayBill.FormClose(Sender: TObject;
...
//------------------------------------------------------------------------------
procedure TfFormPayBill.InitFormData(const nID: string);
begin
...
//Desc: 清理客户信息
procedure TfFormPayBill.ClearCustomerInfo;
begin
  ListInfo.Clear;
  EditIn.Text := '0';
  EditOut.Text := '0';
...
//Desc: 载入nID客户的信息
function TfFormPayBill.LoadCustomerInfo(const nID: string): Boolean;
var nStr: string;
    nDS: TDataSet;
begin
  ClearCustomerInfo;
...
procedure TfFormPayBill.EditSalesManPropertiesChange(Sender: TObject);
var nStr: string;
begin
...
procedure TfFormPayBill.EditNamePropertiesEditValueChanged(Sender: TObject);
begin
...
procedure TfFormPayBill.EditIDPropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
begin
...
//Desc: 选择客户
procedure TfFormPayBill.EditNameKeyPress(Sender: TObject; var Key: Char);
var nStr: string;
...
//------------------------------------------------------------------------------
function TfFormPayBill.OnVerifyCtrl(Sender: TObject; var nHint: string): Boolean;
begin
  Result := True;
...
procedure TfFormPayBill.BtnOKClick(Sender: TObject);
var nP: TFormCommandParam;
begin
  if not IsDataValid then Exit;
  if not SaveCustomerPayBill(gInfo.FCusID, gInfo.FCusName,
     GetCtrlData(EditSalesMan), sFlag_MoneyHuiKuan, EditType.Text, EditDesc.Text,
     StrToFloat(EditMoney.Text),Date2str(EditKPDate.Date)) then
  begin
    ShowMsg('发票操作失败', sError); Exit;
  end;
  ModalResult := mrOk;
  ShowMsg('发票操作成功', sHint);
end;

initialization
  gControlManager.RegCtrl(TfFormPayBill, TfFormPayBill.FormID);
end.
