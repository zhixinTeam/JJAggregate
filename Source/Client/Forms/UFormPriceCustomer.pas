...
type
  TfFormPriceCusomter = class(TfFormNormal)
...
class function TfFormPriceCusomter.CreateForm(const nPopedom: string;
...
begin
  ...
  with TfFormPriceCusomter.Create(Application) do
  begin
    if Assigned(nP) then
    begin
      FDefaultWeek := nP.FParamA;
      FActiveCusID := nP.FParamB;
      FActiveCusName := nP.FParamC;
    end else
    begin
      FDefaultWeek := '';
      FActiveCusID := '';
...
class function TfFormPriceCusomter.FormID: integer;
begin
  Result := cFI_FormPriceCustomer;
end;

procedure TfFormPriceCusomter.FormCreate(Sender: TObject);
begin
...
procedure TfFormPriceCusomter.FormClose(Sender: TObject;
...
//------------------------------------------------------------------------------
procedure TfFormPriceCusomter.InitFormData(const nID: string);
var nStr: string;
    nIdx,nInt: Integer;
begin
  List1.Clear;
  ...
  if FActiveCusID <> '' then
    EditCustomer.Text := Format('%s.%s', [FActiveCusID, FActiveCusName]);
  //xxxxx
  nInt := 0;
...
procedure TfFormPriceCusomter.EditWeekPropertiesChange(Sender: TObject);
var nStr: string;
    nIdx: Integer;
begin
  ...
  begin
    ...
    FChanged := False;
    if FActiveCusID = '' then Exit;
    GetLadingStockItems(gStockTypes);

    nStr := 'Select R_StockNo,R_Price From %s ' +
            'Where R_Type=''%s'' And R_Week=''%s'' And R_Customer=''%s''';
    nStr := Format(nStr, [sTable_PriceRule, sFlag_PriceZY, FID, FActiveCusID]);
  end;
...
procedure TfFormPriceCusomter.LoadStockList;
var nStr: string;
    nIdx: Integer;
begin
...
procedure TfFormPriceCusomter.ShowPriceEditor(const nShow: Boolean);
begin
...
procedure TfFormPriceCusomter.List1DblClick(Sender: TObject);
begin
  if Assigned(List1.Selected) then
  begin
...
procedure TfFormPriceCusomter.BtnNoClick(Sender: TObject);
begin
  ShowPriceEditor(False);
end;

procedure TfFormPriceCusomter.BtnYesClick(Sender: TObject);
var nIdx: Integer;
    nVal: Double;
begin
...
procedure TfFormPriceCusomter.List1KeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
...
procedure TfFormPriceCusomter.EditPricePropertiesButtonClick(Sender: TObject;
  AButtonIndex: Integer);
var nIdx: Integer;
begin
  EditPrice.Text := '0';
...
procedure TfFormPriceCusomter.EditCustomerPropertiesButtonClick(
...
//Desc: 保存
procedure TfFormPriceCusomter.BtnOKClick(Sender: TObject);
var nStr: string;
    nIdx: Integer;
begin
  ...
  try
    ...
    begin
      ...
      nStr := MakeSQLByStr([SF('R_Week', gPriceWeeks[FActiveWeek].FID),
              SF('R_Type', sFlag_PriceZY),
              SF('R_Customer', FActiveCusID),
              SF('R_StockNo', FID),
...
initialization
  gControlManager.RegCtrl(TfFormPriceCusomter, TfFormPriceCusomter.FormID);
end.
